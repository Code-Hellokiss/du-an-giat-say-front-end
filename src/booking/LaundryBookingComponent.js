...
import {createPaymentVnpay} from "../services/VnpayService";
...
const LaundryBookingComponent = () => {
    ...
    const validateForm = () => {
        const errors = {};
        if (!address || address.trim().length < 4) errors.address = 'Vui lòng nhập địa chỉ cụ thể!';
        if (!pickupTime) errors.pickupTime = 'Vui lòng chọn thời gian lấy đồ!';
        if (!deliveryTime) errors.deliveryTime = 'Vui lòng chọn thời gian giao đồ!';
        if (!shipperId) errors.shipperId = 'Vui lòng chọn shipper!';
        if (items.length === 0) errors.items = 'Vui lòng thêm ít nhất một mục giặt!';
        else {
            items.forEach((item, index) => {
                if (!item.categoryId) errors[`itemCategory${index}`] = `Chọn loại đồ cho mục ${index + 1}`;
                if (item.quantity <= 0) errors[`itemQuantity${index}`] = `Số kg của mục ${index + 1} phải > 0`;
            });
        }
        ...
        if (pickup <= new Date()) errors.pickupTime = 'Thời gian lấy đồ phải là tương lai!';
        if (diffHours < 12) errors.deliveryTime = 'Giao đồ phải sau lấy đồ ít nhất 12 giờ!';
    ...
    const handlePaymentClick = async () => {
        try {
            ...
            if (paymentMethod === 'PAYPAL') {
                response = await createPayment(calculateTotal(), createdOrderId); // Handle PayPal payment
            } else if (paymentMethod === 'VNPAY') {

                response = await createPaymentVnpay(calculateTotal(), createdOrderId);
            }
        ...
        <Box p={3} maxWidth="md" mx="auto">
            <Paper elevation={3} sx={{ p: 4, borderRadius: 3 }}>
                ...
                <Grid container spacing={2}>
                    ...
                    <Grid item xs={12}>
                        <Button fullWidth variant="contained" onClick={handleSubmit} sx={{ py: 1.2 }}>
                            ĐẶT LỊCH GIẶT NGAY
                        </Button>
                    </Grid>
                    {showPaymentButton && (paymentMethod === 'PAYPAL' || paymentMethod === 'VNPAY') && (
                        <Grid item xs={12}>
